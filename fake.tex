...
\end{lstlisting}
	...
	\begin{homeworkProblem}[Part 3]
		...
		\begin{homeworkProblem}[Part 3a]
			...
			The strong presence of words in real news (words that appears a lot)  and absence in fake news strongly predicts that the news is real. The strong presence of words in fake news and absence in real news strongly predicts that the news is fake.
...
Without normalization, words with small magnitude would have more effects on output, or in other words, are more important than they are supposed to be in parameter numeric values. This would have bad influence because parameters value do not make that much sense any more as normalized ones because there are some bias produced by magnitude issues. 
...
>>>>>>> 2fcfde4e247687ab95fee9fe80aee69547f77eeb
	%----------------------------------------------------------------------------------------
	%	PROBLEM 7
	%----------------------------------------------------------------------------------------
...
Max\_depth is the tuned parameter. 
\\
Best max\_depth is 150 on condition that it reaches the best training performance at once.  When max\_dept = 800, we have the best validation performance. Max\_depth = 150 is the best parameter in my opinion because the training process afterwards does not actually improve training performance, it only increases the possibility of overfit. 
\\
I tried different values in max\_leaf\_nodes. It reduced the training set accuracy and improved better results in validation set and test set.

\begin{figure*}[!ht]
\centering
\includegraphics[scale = 0.7 ]{Depths_and_performances_part7a_.png}
\caption{Depths and performances(part7a)}
\label{fig:part7a graph}
\end{figure*}
...
\begin{figure*}[!ht]
\centering
\includegraphics[scale = 0.7]{Decision_tree_part7b_.JPG}
\caption{Visualization of decision tree}
\label{fig:Decision Tree}
\end{figure*}

Common words in decision tree as the ones in Part 3a and Part 6a: "donald", "hillary", "trumps", "trump", "the" appear as the most important features both in decision tree and Naive Bayes; "donald", "hillary", "trumps" appear as the most important features both in decision tree and Logistic Regression. 

\noindent \textbf{\textit{Part 7c)}}
Training set performance: logistic regression>Decision Tree>Naive Bayers\\
Validation set performance: Naive Bayers>logistic regression>Decision Tree\\
Test set performance: logistic regression>Naive Bayers>Decision Tree\\

In a nutshell, naive bayers and logistic regression are both good model because of their overall performance. Decision Tree model is worse than those two models. It performs badly due to overfitting issues.
...
\noindent \textit{Mutual information}

\subsection{(a) Mutual information, first split}
\begin{align}
I(Y; x_{470}) &= H(Y) = H(Y | x_{470}) \\
H(Y) &= - \frac{909}{288} \log_{2} (\frac{909}{288}) -
\frac{379}{288} \log_{2} (\frac{379}{288}) = 0.476 \\
H(Y|x_{5170}=0) &= - \frac{96}{1909} \log_{2} (\frac{96}{1909}) -
\frac{1813}{1909} \log_{2} (\frac{1813}{1909}) = 0.2876 \\
H(Y|x_{5170}=1) &= - \frac{15}{379} \log_{2} (\frac{15}{379}) -
\frac{364}{379} \log_{2} (\frac{364}{379}) = 0.2404 \\
H(Y|x_{5170}) &= \frac{1909}{2288} H(Y|x_{5170}=0) + \frac{379}{2288} H(Y|x_{5170}=1)
= 0.2798 \\
I(Y; x_{5170}) &= 0.6476 - 0.2798 = 0.3678
\end{align}

\subsection{(b) Mutual information, continued}

\begin{align}
I(Y; x_{5351}) &= H(Y) = H(Y | x_{5351}) \\
H(Y) &= - \frac{364}{379} \log_{2} (\frac{364}{379}) -
\frac{15}{379} \log_{2} (\frac{15}{379}) = 0.2404 \\
H(Y|x_{5351}=0) &= - \frac{81}{364} \log_{2} (\frac{81}{364}) -
\frac{283}{364} \log_{2} (\frac{283}{364}) = 0.7648 \\
H(Y|x_{5351}=1) &= - \frac{1}{15} \log_{2} (\frac{1}{15}) -
\frac{14}{15} \log_{2} (\frac{14}{15}) = 0.3534 \\
H(Y|x_{5351}) &= \frac{364}{379} H(Y|x_{5351}=0) + \frac{15}{379} H(Y|x_{5351}=1)
= 0.7485 \\
I(Y; x_{5351}) &= 0.2404 - 0.7485 = -0.5081
\end{align}
