...
\begin{document}
	...
	\begin{homeworkProblem}[ Part 2]
		...
		P(real) = total number of real words / 
		
		
		
		headline = [word\_1, word\_2, word\_3...word\_n]
		
		We used this formula: 
		
		likelihood\_probability\_headline 
		
		= P(word\_1 $\mid$ real) * P(word\_2 $\mid$ real) * P(word\_3 $\mid$ real)  * ... * P(word\_n $\mid$ real)
		
		= log(P(word\_1 $\mid$ real)) + log(P(word\_2 $\mid$ real)) + log(P(word\_3 $\mid$ real)) ... + log(P(word\_n $\mid$ real)) 
